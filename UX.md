...
- **Every focusable panel's border tag is its literal go-to combo**: `[/]`
  Search (`/`), `[Alt+C]` Chats, `[Alt+M]` Messages, `[Alt+F]` in-chat search,
  `[Alt+L]` command Log. The `Alt+letter` jumps are **global** — they fire
  from any focus, even mid-compose (weechat-style modifier chords), so the tag
  always tells the truth. `Ctrl+F` is also accepted for the chat search.
  Re-homed to free the section letters: copy label `Alt+Y` (yank), mark read
  `Alt+S` (seen). `Tab`/`Shift+Tab` also cycle focus via
...
its panels `─[/]-Search`, `─[Alt+C]-Chats`, `─[Alt+M]-Chat`, `─[Alt+L]-Command log`; Teams
